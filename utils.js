/**
 *
 * @description Fileters and formats raw data from PCO into structure required by Data Studio
 * @param {array} [selectedFields=[]] attributes user selected to display
 * @param {array} [rawApiData=[]] raw responses from PCO API
        ...
        .map((row) => {
          ...
          Object.keys(row.attributes).forEach((attribute) => {
            if (selectedFields.indexOf(attribute) !== -1) {
              ...
              if (
                ENDPOINTS[selectedAPI].attributes[attribute] === "timestamp"
              ) {
...
/**
 *
 * @description Creates schema for the selected API
 * @param {object} selectedApiConfig config object for selected API
 * @return {object} Data Studio fields
 */
const deriveSchema = (selectedApiConfig) => {
  ...
  Object.keys(attributes).forEach((prop) => {
    ...
    switch (attributes[prop]) {
      case "boolean":
        field = fields.newDimension().setId(prop).setType(types.BOOLEAN);
        break;
      case "tinyint":
      case "smallint":
      case "int":
      ...
      case "float":
      case "decimal":
        field = fields.newMetric().setId(prop).setType(types.NUMBER);
        break;
      case "char":
      case "varchar":
      case "string":
        field = fields.newDimension().setId(prop).setType(types.TEXT);
        break;
      case "date":
        field = fields.newDimension().setId(prop).setType(types.YEAR_MONTH_DAY);
        break;
      case "timestamp":
        field = fields
          .newDimension()
...
/**
 *
 * @description request.configParams filter for user specified parameters
 * @param {*} [configParams={}] request.configParams
 * @return {object} user defined parameters
 */
const filterUserDefinedParams = (configParams = {}) => {
  ...
  return Object.keys(configParams).reduce((acc, curr) => {
    if (
      typeof configParams[curr] === "string" &&
      typeof curr.split("_")[1] !== "undefined"
    ) {
...
/**
 *
 * @description Recursive function that fetches API data from PCO
 * @param {string} selectedAPI selected API
 * @param {object} [options={}] fetch options
 * @return {array} array of API data objects
 */
const requestPCO = (selectedAPI, options = {}) => {
  ...
  if (
    !options.hasOwnProperty("muteHttpExceptions") ||
    typeof options.muteHttpExceptions !== "boolean"
  ) {
  ...
  if ([ENDPOINTS[selectedAPI].status_code, 429].indexOf(responseCode) >= 0) {
    if (responseCode === 429 && responseHeaders.hasOwnProperty("Retry-After")) {
      console.warn(
        "rate limiter applied. retry after: ",
        parseInt(responseHeaders["Retry-After"], 10) * 1000
      );
